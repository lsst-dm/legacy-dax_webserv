...
def flask_listener():
    ...
    if not capabilities:
        capabilities = [get_capability(resource, request_method)]
...
def get_capability(resource: str, request_method: str) -> Optional[str]:
    """
...
def get_key_as_pem(request_issuer, request_key_id):
    ...
    pem_keys = {}
    for issuer_uri, info in Config.AUTHORIZED_ISSUERS.items():
        oidc_config = os.path.join(issuer_uri, ".well-known/openid-configuration")
        try:
            ...
            keys_map = {k["kid"]: k for k in keys}
            key = keys_map[info["issuer_key_id"]]
            ...
            pem_keys.setdefault(issuer_uri, {})[key["kid"]] = _convert(e, m)
        except (KeyError, HTTPError) as e:
            logger.error(f"Unable to store key for issuer: {issuer_uri} ")
            logger.error(e)
            raise e
